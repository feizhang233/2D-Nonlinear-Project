import type { HoleShape } from './cadGeometry'
import type { LoadPlacementKind } from './loadPlacement'
import { applyDefaultSection } from './sections'
...
export const isSurfaceFamily = (model: ModelInput) =>
...
export function parseSketch(
  ...
  const vertices = Array.isArray(value.vertices)
    ? value.vertices.flatMap((item) => {
        if (!isRecord(item) || typeof item.id !== 'string') return []
        const coordinates = Array.isArray(item.coordinates)
          ? item.coordinates
              .map(Number)
              .filter((entry) => Number.isFinite(entry))
          : [Number(item.x), Number(item.y)].filter((entry) =>
        ...
        return [
          {
            ...
            vertexIds,
            ...(isRecord(item.shape) &&
            ['circle', 'rectangle', 'square'].includes(
              String(item.shape.kind),
            ) &&
            Array.isArray(item.shape.center) &&
            item.shape.center.length === 2 &&
            item.shape.center.every(
              (n) => typeof n === 'number' && Number.isFinite(n),
            )
              ? { shape: item.shape as unknown as HoleShape }
              : {}),
          },
        ]
      })
    : []
  if (!vertices.length || !loops.some((loop) => loop.kind === 'outer'))
    return null
...
export function deriveSketch(model: ModelInput): SketchGeometry {
  ...
  const exterior = [...edgeCount.values()]
  ...
  if (adjacency.size) {
    const origin = [...adjacency.keys()].sort((left, right) => {
      const a = model.nodes.find((node) => node.id === left)?.coordinates ?? [
        0, 0,
      ]
      const b = model.nodes.find((node) => node.id === right)?.coordinates ?? [
        0, 0,
      ]
  ...
  const vertexIds = collapseCollinear(
    uniqueOrdered(
      ordered.length >= 3 ? ordered : model.nodes.map((node) => node.id),
    ),
    lookup,
  )
...
export function getSketch(model: ModelInput): SketchGeometry {
  return (
    parseSketch(model.extensions?.geometry as JsonValue | undefined) ??
    deriveSketch(model)
  )
}
...
export function isConvexQuad(points: number[][]): boolean {
  ...
  return turns.every((value) => value > 1e-12)
}
...
export function geometryNeedsMesh(model: ModelInput): boolean {
  ...
  if (
    !outer ||
    outer.vertexIds.length !== 4 ||
    !isConvexQuad(loopPoints(sketch, outer))
  )
    return true
...
const vertexMap = (sketch: SketchGeometry) =>
  new Map(sketch.vertices.map((vertex) => [vertex.id, vertex]))

const collapseCollinear = (
  ids: string[],
  lookup: Map<string, number[]>,
): string[] => {
  const points = ids.map((id) => ({
    id,
    coordinates: lookup.get(id) ?? [0, 0],
  }))
  let changed = true
  while (changed && points.length > 4) {
    ...
    for (let index = 0; index < points.length; index += 1) {
      const before =
        points[(index + points.length - 1) % points.length].coordinates
      ...
      const scale = Math.max(
        Math.hypot(left[0], left[1]) * Math.hypot(right[0], right[1]),
        1,
      )
...
const pointSegmentDistance = (
  ...
  const parameter =
    length2 <= 0 ? 0 : Math.max(0, Math.min(1, (wx * vx + wy * vy) / length2))
...
export function nearestSketchVertex(
  sketch: SketchGeometry,
  coordinates: number[],
): SketchVertex | null {
  ...
  return sketch.vertices.reduce((best, vertex) => {
    const bestDistance =
      (best.coordinates[0] - coordinates[0]) ** 2 +
      (best.coordinates[1] - coordinates[1]) ** 2
    const nextDistance =
      (vertex.coordinates[0] - coordinates[0]) ** 2 +
      (vertex.coordinates[1] - coordinates[1]) ** 2
...
export function nearestModelNode(
  ...
  return model.nodes.reduce((best, node) => {
    const bestDistance =
      ...
      (best.coordinates[1] - coordinates[1]) ** 2
    const nextDistance =
      (node.coordinates[0] - coordinates[0]) ** 2 +
      (node.coordinates[1] - coordinates[1]) ** 2
    return nextDistance < bestDistance ? node : best
  })
}

export function nodeForSketchVertex(
  model: ModelInput,
  vertex: SketchVertex,
): NodeInput | undefined {
  const tagged = model.nodes.find(
    (node) => node.extensions?.geometry_vertex_id === vertex.id,
  )
  const node = tagged ?? nearestModelNode(model, vertex.coordinates)
  return node &&
    Math.hypot(
      node.coordinates[0] - vertex.coordinates[0],
      node.coordinates[1] - vertex.coordinates[1],
    ) < 1e-8
    ? node
    : undefined
}
...
export function addOuterVertex(
  ...
  const id = nextPrefixedId(
    'G',
    sketch.vertices.map((vertex) => vertex.id),
  )
  const outer = sketch.loops.find((loop) => loop.kind === 'outer')
  ...
  const next: SketchGeometry = {
    ...
    loops: outer
      ? sketch.loops.map((loop) =>
          loop.kind === 'outer' ? { ...loop, vertexIds } : loop,
        )
...
export function addOuterVertexAt(
  model: ModelInput,
  coordinates: number[],
): ModelInput {
  const sketch = getSketch(model)
  const outer = sketch.loops.find((loop) => loop.kind === 'outer')
  if (!outer || outer.vertexIds.length < 2)
    return addOuterVertex(model, coordinates)
  const vertices = vertexMap(sketch)
  ...
  for (let index = 0; index < outer.vertexIds.length; index += 1) {
    ...
    const end = vertices.get(
      outer.vertexIds[(index + 1) % outer.vertexIds.length],
    )?.coordinates ?? [0, 0]
...
export function addRectangularHole(
  model: ModelInput,
  center: number[],
  size: number,
): ModelInput {
...
export function moveSketchVertex(
  model: ModelInput,
  vertexId: string,
  coordinates: number[],
): ModelInput {
  ...
  const next = {
    ...sketch,
    loops: sketch.loops.map((loop) =>
      loop.vertexIds.includes(vertexId) ? { ...loop, shape: undefined } : loop,
    ),
    vertices: sketch.vertices.map((vertex) =>
...
export function deleteSketchVertex(
  model: ModelInput,
  vertexId: string,
): ModelInput {
  const sketch = getSketch(model)
  const nextLoops = sketch.loops
    .map((loop) => ({
      ...loop,
      vertexIds: loop.vertexIds.filter((id) => id !== vertexId),
    }))
...
export function deleteSketchLoop(
  model: ModelInput,
  loopId: string,
): ModelInput {
  const sketch = getSketch(model)
...
function rebuildQuadIfPossible(
  model: ModelInput,
  sketch: SketchGeometry,
): ModelInput {
  const outer = sketch.loops.find((loop) => loop.kind === 'outer')
  if (
    !outer ||
    outer.vertexIds.length !== 4 ||
    sketch.loops.some((loop) => loop.kind === 'hole')
  ) {
    return model
  }
  if (!isConvexQuad(loopPoints(sketch, outer))) return model
...
export function addFrameNode(
  model: ModelInput,
  coordinates: number[],
): { model: ModelInput; nodeId: string } {
  const id = nextPrefixedId(
    'N',
...
export function addFrameMember(
  ...
  const start = model.nodes.find((node) => node.id === startId)
  const end = model.nodes.find((node) => node.id === endId)
  if (
    !start ||
    !end ||
  ...
  if (
    ...
    return model
  const familyFormulation =
    model.elements[0]?.formulation ?? 'frame2d-corotational'
  const properties = structuredClone(
    model.elements[0]?.properties ?? { area: 0.01, second_moment: 1e-8 },
  )
  const id = nextPrefixedId(
    'E',
    model.elements.map((element) => element.id),
  )
  return applyDefaultSection(
    {
      ...model,
      elements: [
        ...model.elements,
...
export function placeFramePoint(
  model: ModelInput,
  coordinates: number[],
  tolerance: number,
) {
  const existing = model.nodes.find(
    (node) =>
      Math.hypot(
  ...
  return existing
    ? { model, nodeId: existing.id }
    : addFrameNode(model, coordinates)
}

export function moveFrameNode(
  model: ModelInput,
  nodeId: string,
  coordinates: number[],
): ModelInput {
...
export function editablePlacementNodes(
  model: ModelInput,
): Array<{ id: string; label: string; coordinates: number[] }> {
  if (isSurfaceFamily(model)) {
    const sketch = getSketch(model)
    if (geometryNeedsMesh(model)) return []
    return sketch.vertices.flatMap((vertex, index) => {
...
export function firstFreePlacementNodeId(
  model: ModelInput,
): string | undefined {
